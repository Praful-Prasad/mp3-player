...
def add_song():
	...
	for i in range(len(song)-1,-1,-1):
		if(song[i]=="/"):
			break
	global song_path
	song_path=song[0:i+1]
	song = song.replace(song[:i+1],"")
...
# Create Playlist Box
playlist_box = Listbox(main_frame, bg="grey", fg="black", width=190,height = 30, selectbackground="green", selectforeground='black')
...
back_btn_img = PhotoImage(file='images/skiptoend.png')
forward_btn_img = PhotoImage(file='images/fastforward.png')
